...
public class Application {
	public static void listMultiply(List<Integer> list1, List<Integer> list2) {
		...
		IntStream.range(0, Math.min(list1.size(), list2.size())).forEach(index -> result.add(list1.get(index) * list2.get(index)));
	...
	public static void main(String[] args) {
		...
		List<List<Object>> wordCountList = new LinkedList<>();
		wordCountMap.forEach((key, value) -> {
			wordCountList.add(Arrays.asList(key, value));
		});
